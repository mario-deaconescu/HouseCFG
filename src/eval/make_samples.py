...
def make_samples(model, output_path: str, num_samples: int = 1000, batch_size: int = 1, num_timesteps: int = 1000,
    ...
    while generated < num_samples:
        samples = sample_plan(diffusion, model, num_samples=batch_size, device=device, data_path=data_path,
                              mask_size=mask_size,
        ...
        process_map(save_img_plan, [output_path] * len(plans), plans, range(len(plans)), [epoch] * len(plans),
                    max_workers=num_workers, chunksize=100, desc=f"Generating Step {epoch}: {generated}/{num_samples}", total=len(plans))
        generated += len(plans)
        epoch += 1
