...
class CFGUnet(nn.Module):
    ...
    def __init__(
        ...
        self.classes_emb = nn.Linear(RoomType.restricted_length() + 1, dim)
        ...
        **kwargs
        ...
        if self.use_bubbles:
            ...
            if bubbles is None:
                ...
                cond_flags = torch.ones(bubbles.shape, device=x.device, dtype=torch.bool)
...
class CFGUnetWithScale(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        logits = self.unet(*args, cond_drop_prob = 0., **kwargs)

        if cond_scale == 1:
            return logits

        null_logits = self.unet(*args, cond_drop_prob = 1., **kwargs)
