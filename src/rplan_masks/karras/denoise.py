...
from src.rplan_masks.karras.attend import Attend
...
class GithubUnet(Module):
    def __init__(
        ...
        self.condition_drop_prob = codition_drop_prob
    ...
    def forward(self, x, time, x_self_cond = None, cond_drop_prob:Optional[int]=None, **kwargs):
        ...
        # x = torch.cat((x, src_key_padding_mask.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, x.shape[2], x.shape[3])), dim = 1)
        x = torch.cat((x, masks, bubbles), dim = 1)
