...
@torch.no_grad()
def sample_plans_combined(diffusion: GaussianDiffusion, model, num_samples: int = 1, data_path='data/rplan',
                          mask_size: int = 64,
                          bubbles: Optional[np.ndarray] = None, room_types: Optional[np.ndarray] = None,
    ...
    if bubbles is not None:
        assert room_types is not None, "If bubbles are provided, room_types must also be provided."
    ...
    images, walls, doors, room_types, bubbles, masks = images.to(device), walls.to(device), doors.to(
        device), room_types.to(device), bubbles.to(device), masks.to(device)
    ...
    model_kwargs = {
        'masks': masks,
        'room_types': room_types if force_bubbles or (input_bubbles is not None) else None,
    ...
    return {
        "samples": samples,
        "bubbles": bubbles.cpu().numpy(),
        "room_types": room_types.cpu().numpy() if room_types is not None else None,
    }
